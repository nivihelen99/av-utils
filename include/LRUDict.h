...
public:
    ...
    using value_type = std::pair<const Key, Value>; // Type stored in the list
    using hasher = Hash;
...
public:
    ...
    std::pair<iterator, bool> insert_or_assign(Key&& k, const Value& v);
    std::pair<iterator, bool> insert_or_assign(Key&& k, Value&& v);
    ...
    std::optional<std::reference_wrapper<const Value>> peek(const Key& key) const;
...
//private: // Making public for LRUDict to access list_iter_ directly in erase for now.
// Better to keep private and add a getter or make LRUDictIterator a full friend of LRUDict.
// For now, this is simpler for erase(iterator) implementation.
private:
    internal_list_iterator_type list_iter_;

    friend LRUDictType; // LRUDict can access private members of its iterator
    friend class LRUDictIterator<LRUDictType, true>; // Const_iterator can access non-const version's members for construction
...
void LRUDict<K, V, H, KE, A>::_evict() {
    if (capacity_ == 0 || list_.empty()) return; // Cannot evict if capacity 0 or empty
...
template <typename K, typename V, typename H, typename KE, typename A>
LRUDict<K, V, H, KE, A>::LRUDict(size_type cap,
                                 const hasher& hash,
                                 const key_equal& equal,
                                 const allocator_type& alloc)
    : list_(alloc),
      map_( (cap > 0 ? cap : 10), hash, equal, map_value_allocator(alloc)), // Use rebinded allocator
      capacity_(cap) {
    if (capacity_ > 0) {
...
LRUDict<K, V, H, KE, A>::LRUDict(const LRUDict& other)
    : list_(std::allocator_traits<list_value_allocator>::select_on_container_copy_construction(other.list_.get_allocator())),
      map_(other.map_.bucket_count(), other.map_.hash_function(), other.map_.key_eq(),
           std::allocator_traits<map_value_allocator>::select_on_container_copy_construction(other.map_.get_allocator())),
      capacity_(other.capacity_) {
    if (capacity_ > 0) map_.rehash( static_cast<size_t>(capacity_ / map_.max_load_factor()) +1 );
    for (auto it = other.list_.rbegin(); it != other.list_.rend(); ++it) {
        list_.push_front(*it);
        map_.emplace(list_.front().first, list_.begin());
    }
}
...
LRUDict<K, V, H, KE, A>::LRUDict(const LRUDict& other, const allocator_type& alloc)
    : list_(alloc),
      map_(other.map_.bucket_count(), other.map_.hash_function(), other.map_.key_eq(), map_value_allocator(alloc)),
      capacity_(other.capacity_) {
    if (capacity_ > 0) map_.rehash( static_cast<size_t>(capacity_ / map_.max_load_factor()) +1 );
    for (auto it = other.list_.rbegin(); it != other.list_.rend(); ++it) {
        list_.push_front(*it);
...
LRUDict<K, V, H, KE, A>::LRUDict(LRUDict&& other, const allocator_type& alloc) noexcept
    : list_(std::move(other.list_), alloc),
      map_(std::move(other.map_)), // This move needs to handle allocator if map supports it
      capacity_(other.capacity_) {
    // If map's allocator is not compatible with 'alloc' and map doesn't handle it in move,
    // we might need to rebuild the map.
    // std::unordered_map's move constructor taking an allocator is C++20.
    // If map_ was moved but its iterators are now invalid for the (potentially also moved with different alloc) list_,
    // a rebuild is necessary.
    // This is a complex part of allocator awareness. For simplicity, assume map move is okay or rebuild.
    if (map_.get_allocator() != map_value_allocator(alloc)) { // A simplified check
        map_type temp_map(capacity_ > 0 ? capacity_ : 10, other.hash_function(), other.key_eq(), map_value_allocator(alloc));
        if (capacity_ > 0) temp_map.rehash( static_cast<size_t>(capacity_ / temp_map.max_load_factor()) +1 );
        // Populate temp_map from list_
        for(auto it = list_.begin(); it != list_.end(); ++it) {
            temp_map.emplace(it->first, it);
        }
...
LRUDict<K, V, H, KE, A>& LRUDict<K, V, H, KE, A>::operator=(LRUDict&& other) noexcept {
    if (this == &other) return *this;
    // Clear current state and move from other, handling allocators.
    // This simplified version relies on member-wise swap for correctness if allocators compatible.
    // A full POCMA-aware assignment is more involved.
    LRUDict temp(std::move(other)); // Use move constructor
    swap(temp); // Our swap is noexcept
    return *this;
}
...
LRUDict<K, V, H, KE, A>::insert(const value_type& kv_pair) {
    if (capacity_ == 0) return {end(), false};
    auto map_it = map_.find(kv_pair.first);
    if (map_it != map_.end()) {
        map_it->second->second = kv_pair.second;
        _move_to_front(map_it->second);
        return {iterator(map_it->second), false};
    } else {
        if (is_full()) _evict();
        list_.push_front(kv_pair);
        map_.emplace(kv_pair.first, list_.begin());
        return {iterator(list_.begin()), true};
    }
}
...
LRUDict<K, V, H, KE, A>::insert(value_type&& kv_pair) {
    ...
    auto map_it = map_.find(kv_pair.first);
    if (map_it != map_.end()) {
        map_it->second->second = std::move(kv_pair.second);
        _move_to_front(map_it->second);
        return {iterator(map_it->second), false};
    } else {
        if (is_full()) _evict();
        list_.push_front(std::move(kv_pair));
...
template <typename K, typename V, typename H, typename KE, typename A>
std::pair<typename LRUDict<K, V, H, KE, A>::iterator, bool>
LRUDict<K, V, H, KE, A>::insert_or_assign(const Key& k, const Value& v) {
    auto map_it = map_.find(k);
    if (map_it != map_.end()) {
        map_it->second->second = v;
        _move_to_front(map_it->second);
        return {iterator(map_it->second), false};
    } else {
        if (capacity_ == 0) return {end(), false};
...
template <typename K, typename V, typename H, typename KE, typename A>
std::pair<typename LRUDict<K, V, H, KE, A>::iterator, bool>
LRUDict<K, V, H, KE, A>::insert_or_assign(const Key& k, Value&& v) {
    auto map_it = map_.find(k);
    if (map_it != map_.end()) {
        map_it->second->second = std::move(v);
        _move_to_front(map_it->second);
        return {iterator(map_it->second), false};
    } else {
        if (capacity_ == 0) return {end(), false};
...
template <typename K, typename V, typename H, typename KE, typename A>
std::pair<typename LRUDict<K, V, H, KE, A>::iterator, bool>
LRUDict<K, V, H, KE, A>::insert_or_assign(Key&& k, const Value& v) {
    // Find using a temporary if Key might be same as value in map_it->second->first
    // For safety, copy key for map find if it's an rvalue.
    key_type temp_k = k;
    ...
    if (map_it != map_.end()) {
        ...
        if (capacity_ == 0) return {end(), false};
        if (is_full()) _evict();
        list_.emplace_front(std::move(k), v); // k is moved here
        map_.emplace(list_.begin()->first, list_.begin()); // key from list used for map
        return {iterator(list_.begin()), true};
    }
}
...
template <typename K, typename V, typename H, typename KE, typename A>
std::pair<typename LRUDict<K, V, H, KE, A>::iterator, bool>
LRUDict<K, V, H, KE, A>::insert_or_assign(Key&& k, Value&& v) {
    key_type temp_k = k;
    auto map_it = map_.find(temp_k);
    if (map_it != map_.end()) {
        map_it->second->second = std::move(v);
...
template <typename... Args>
std::pair<typename LRUDict<K, V, H, KE, A>::iterator, bool>
LRUDict<K, V, H, KE, A>::try_emplace(const Key& k, Args&&... args) {
    auto map_it = map_.find(k);
    if (map_it != map_.end()) {
        _move_to_front(map_it->second);
        return {iterator(map_it->second), false};
        ...
        if (capacity_ == 0) return {end(), false};
        if (is_full()) _evict();
        list_.emplace_front(std::piecewise_construct,
                            std::forward_as_tuple(k),
                            std::forward_as_tuple(std::forward<Args>(args)...));
        map_.emplace(k, list_.begin()); // Use k directly as it's const Key&
        return {iterator(list_.begin()), true};
    }
}
...
template <typename... Args>
std::pair<typename LRUDict<K, V, H, KE, A>::iterator, bool>
LRUDict<K, V, H, KE, A>::try_emplace(Key&& k, Args&&... args) {
    // Need to check find before k is potentially moved.
    // If k is not copyable, this is tricky. Assume k is copyable for find, or use list_begin()->first.
    key_type temp_k = k;
    auto map_it = map_.find(temp_k);
    if (map_it != map_.end()) {
        _move_to_front(map_it->second);
        ...
        list_.emplace_front(std::piecewise_construct,
                            ...
                            std::forward_as_tuple(std::forward<Args>(args)...));
        map_.emplace(list_.begin()->first, list_.begin()); // Key from list after move
        return {iterator(list_.begin()), true};
    }
}
...
LRUDict<K, V, H, KE, A>::operator[](const Key& key) {
    ...
    if (map_it != map_.end()) {
        ...
        if (capacity_ == 0) {
            throw std::out_of_range("LRUDict::operator[]: Cannot insert with zero capacity.");
        }
        if (is_full()) _evict();
        list_.emplace_front(key, mapped_type{});
...
template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::mapped_type&
LRUDict<K, V, H, KE, A>::operator[](Key&& key) {
    key_type temp_k = key; // Use copy for find
    auto map_it = map_.find(temp_k);
    if (map_it != map_.end()) {
        _move_to_front(map_it->second);
        return map_it->second->second;
        ...
        if (is_full()) _evict();
        list_.emplace_front(std::move(key), mapped_type{});
        map_.emplace(list_.begin()->first, list_.begin()); // Key from list after move
        return list_.begin()->second;
    }
}
...
template <typename K, typename V, typename H, typename KE, typename A>
bool LRUDict<K, V, H, KE, A>::erase(const Key& key) {
    auto map_it = map_.find(key);
    if (map_it != map_.end()) {
...
LRUDict<K, V, H, KE, A>::erase(iterator pos) {
    ...
    auto next_list_iter = list_.erase(pos.list_iter_); // list_iter_ made public in iterator for this
...
LRUDict<K, V, H, KE, A>::erase(const_iterator pos) {
    ...
    const key_type& key_to_erase = pos.list_iter_->first;
    // std::list::erase takes const_iterator and returns iterator
...
template <typename K, typename V, typename H, typename KE, typename A>
void LRUDict<K, V, H, KE, A>::clear() noexcept {
    map_.clear();
    list_.clear();
}
...
template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::mapped_type&
LRUDict<K, V, H, KE, A>::at(const Key& key) {
    auto map_it = map_.find(key);
    if (map_it != map_.end()) {
        _move_to_front(map_it->second);
        return map_it->second->second;
...
LRUDict<K, V, H, KE, A>::at(const Key& key) const {
    auto map_it = map_.find(key);
...
template <typename K, typename V, typename H, typename KE, typename A>
std::optional<std::reference_wrapper<typename LRUDict<K, V, H, KE, A>::Value>>
LRUDict<K, V, H, KE, A>::get(const Key& key) {
    auto map_it = map_.find(key);
    if (map_it != map_.end()) {
        _move_to_front(map_it->second);
        return std::ref(map_it->second->second);
    }
    return std::nullopt;
}

template <typename K, typename V, typename H, typename KE, typename A>
std::optional<std::reference_wrapper<const typename LRUDict<K, V, H, KE, A>::Value>>
LRUDict<K, V, H, KE, A>::get(const Key& key) const {
    auto map_it = map_.find(key);
...
template <typename K, typename V, typename H, typename KE, typename A>
std::optional<std::reference_wrapper<const typename LRUDict<K, V, H, KE, A>::Value>>
LRUDict<K, V, H, KE, A>::peek(const Key& key) const {
    auto map_it = map_.find(key);
    if (map_it != map_.end()) return std::cref(map_it->second->second);
    return std::nullopt;
}
...
template <typename K, typename V, typename H, typename KE, typename A>
bool LRUDict<K, V, H, KE, A>::contains(const Key& key) const {
    return map_.count(key) > 0;
}

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::iterator
LRUDict<K, V, H, KE, A>::begin() noexcept { return iterator(list_.begin()); }

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::const_iterator
LRUDict<K, V, H, KE, A>::begin() const noexcept { return const_iterator(list_.begin()); }

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::const_iterator
LRUDict<K, V, H, KE, A>::cbegin() const noexcept { return const_iterator(list_.cbegin()); }

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::iterator
LRUDict<K, V, H, KE, A>::end() noexcept { return iterator(list_.end()); }

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::const_iterator
LRUDict<K, V, H, KE, A>::end() const noexcept { return const_iterator(list_.end()); }

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::const_iterator
LRUDict<K, V, H, KE, A>::cend() const noexcept { return const_iterator(list_.cend()); }

template <typename K, typename V, typename H, typename KE, typename A>
typename LRUDict<K, V, H, KE, A>::allocator_type
LRUDict<K, V, H, KE, A>::get_allocator() const {
...
template <typename K, typename V, typename H, typename KE, typename A>
void swap(LRUDict<K, V, H, KE, A>& lhs, LRUDict<K, V, H, KE, A>& rhs) noexcept {
    lhs.swap(rhs);
}
